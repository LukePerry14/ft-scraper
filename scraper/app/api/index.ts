...
async function main() {
  ...
  app.post('/articles', async (req: Request, res: Response) => {
    const articlesJSON = await build_articles_json(context);
  ...
  app.post('/articles/extract', async (req: Request, res: Response) => {

    var articles = req.body?.articles ?? [];

    if (!Array.isArray(articles) || (articles.length > 0 && !articles.every(article => typeof article === "string"))) {
      return res.status(400).send("Invalid Articles format: Expected an array of strings.");
    }

    const articleSet = new Set<string>(articles);

    const FullArticlesJSON = await extract_content(context, articleSet);
